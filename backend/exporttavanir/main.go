...
func Runner(resultFileName, stateList, from, to string) {
	resXlsx, err := excelize.OpenFile("./result99.xlsx")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	if from != "" {
		dateQuery.Must(elastic.NewRangeQuery("event_date").Gte(from))
	}
	if to != "" {
		dateQuery.Must(elastic.NewRangeQuery("event_date").Lte(to))
	}
	...
	qLocationHouse := elastic.NewBoolQuery().Must(
		elastic.NewBoolQuery().Should(
			elastic.NewTermQuery("location_type_id", "1"),
		),
	).Must(elastic.NewExistsQuery("tracking_id")).Must(stateQuery).Must(dateQuery)

	qLocationBiz := elastic.NewBoolQuery().Must(
		elastic.NewBoolQuery().Should(
			elastic.NewTermQuery("location_type_id", "2"),
			elastic.NewTermQuery("location_type_id", "3")),
	).Must(elastic.NewExistsQuery("tracking_id")).Must(stateQuery).Must(dateQuery)
...
func makeCompanisData(q *elastic.BoolQuery) map[string]companyData {
	...
	sumDamageAmountAgg := elastic.NewSumAggregation().Field("amount")

	statusAgg := elastic.NewTermsAggregation().
		Field("status.keyword").
		MinDocCount(0).
		Size(100).
		SubAggregation("amount", sumDamageAmountAgg)

	damageTypeAgg := elastic.NewTermsAggregation().
		Field("compensation_type_id.keyword").
		Size(100).
		MinDocCount(0).
		SubAggregation("compensation_type_id_amount", sumDamageAmountAgg).
	...
	companyAgg := elastic.NewTermsAggregation().
		...
		Size(50).
		MinDocCount(1).
		SubAggregation("compensation_type_id", damageTypeAgg).
		SubAggregation("status", statusAgg)

	res, _ := client.Search().
		Index(elasticIndexName).
		Aggregation("companies", companyAgg).
		Aggregation("compensation_type_id", damageTypeAgg).
		Query(q).
		Size(0).
	...
	for _, r := range coms.Buckets {
		...
		dt, _ := r.Aggregations.Terms("compensation_type_id")
		for _, t := range dt.Buckets {
			switch t.Key.(string) {
			case models.RequestDamageTypeInstrument:
				das, _ := t.Aggregations.Sum("compensation_type_id_amount")
		...
		for _, t := range ds.Buckets {
			das, _ := t.Aggregations.Sum("amount")
			sumDage := cast.ToInt64(das.Value)
...
func makeGeneralReportData(q *elastic.BoolQuery) damagesType {
	...
	dt, _ := res.Aggregations.Terms("compensation_type_id")
	for _, t := range dt.Buckets {
		switch t.Key.(string) {
		case models.RequestDamageTypeInstrument:
			das, _ := t.Aggregations.Sum("compensation_type_id_amount")
...
func getStatusAgg(t *elastic.AggregationBucketKeyItem) map[string]statusData {
	...
	for _, status := range ts.Buckets {
		sum, _ := status.Aggregations.Sum("amount")
